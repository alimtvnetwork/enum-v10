...
All notable changes to **enum-v9** are documented in this file.
...
- **Root cause:** `enum-v9` is a published library. Method/const names like
...
- **Root cause:** `golangci-lint v1.64.8` (last v1.x) is **compiled with
  ...
  whose module declares `go 1.25.0` (which `enum-v9/go.mod` and the
...
## [v1.11.0] — 2026-05-07 — Module rename `enum-v7` → `enum-v9`

### Changed
- Bulk rename of module path and all references from `github.com/alimtvnetwork/enum-v7` to `github.com/alimtvnetwork/enum-v9` across `go.mod`, source, tests, specs, CI workflows, PowerShell tooling, and `.lovable/` memory.
...
### Added
- `spec/05-failing-tests/README.md` — directory-level provenance disclaimer (25 upstream `core-v9` post-mortems; `tests/integratedtests/` paths describe upstream layout, NOT `enum-v9`).
...
### Changed
- `spec/07-code-vs-spec-audits/03-cycle2-error-system.md`: promoted all 7 ❓ rows against upstream `core-v9 v1.5.8` `errcore/`. Verifiable score 11 → **18/18 (100%)**, match rate 10/18 = 55.6%. No new D-CVS findings; spec is accurate, just under-exercised by `enum-v9`.
...
- `Test_AllEnums_NumericRange` line 86 failure is the known PI-006/RCA-Pattern-3 upstream `BasicString` spread defect (no enum-v9 regression; covered by existing skip-list).
...
## [v0.39.0] — 2026-05-07 — Cycle 67 — Bulk rename enum-v6 → enum-v9

### Changed
- Repository renamed from `enum-v6` to `enum-v9` per upstream rename. Bulk substitution applied across `go.mod`, Go source imports, all `spec/`, `.lovable/` memory files, workflows, and `cross-repo/core-v9/README.md` (one-time exception). `.release/`, `.git/`, and the `cross-repo/core-v9/` directory name preserved.
...
- **Cycle 19 spec audit (Task AB pass 1)** — upstream `core-v9 v1.5.8`
  ...
  (M-CVS-01: `enum-v3`→`enum-v9` module name; M-CVS-02: upstream `go.mod`
...
- **Cycle 18 spec audit (Task AA + closes Task AH)** — closed
  ...
  upstream-vs-`enum-v9` scope footnotes on the historical resolution files
...
- **Cycle 15 spec audit (Task AA)** — closed `spec/06-testing-guidelines/`
  ...
  adding an `enum-v9` consumer-coverage callout to `spec/06-testing-guidelines/README.md`
...
- **core-v9 API migration (Task AM)** — Applied all confirmed `core-v9 v1.5.8`
  namespace rewrites across `enum-v9` Go source: `coredynamic.TypeName(...)` →
...
- **`scripts/CoverageRunner.psm1`** — pre-coverage compile check no longer
  ...
  2. The in-package-test scan hard-coded the `core-v9` module prefix when
     ...
     `module` line from `go.mod` so the same script works in `enum-v9`,
...
- Module path migrated from `gitlab.com/auk-go/enum` to
  `github.com/alimtvnetwork/enum-v9`.
