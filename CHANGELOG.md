...
All notable changes to **enum-v10** are documented in this file.
...
## [v1.32.0] - 2026-05-10
### Changed — Module rename enum-v10 → enum-v10
- Bulk rename of `github.com/alimtvnetwork/enum-v10` → `github.com/alimtvnetwork/enum-v10` across `go.mod`, all Go source imports, spec docs, CI workflows, PowerShell tooling, and `.lovable/` memory.
...
- **Root cause:** `enum-v10` is a published library. Method/const names like
...
- **Root cause:** `golangci-lint v1.64.8` (last v1.x) is **compiled with
  ...
  whose module declares `go 1.25.0` (which `enum-v10/go.mod` and the
...
## [v1.11.0] — 2026-05-07 — Module rename `enum-v10` → `enum-v10`

### Changed
- Bulk rename of module path and all references from `github.com/alimtvnetwork/enum-v10` to `github.com/alimtvnetwork/enum-v10` across `go.mod`, source, tests, specs, CI workflows, PowerShell tooling, and `.lovable/` memory.
...
### Changed
- **`.github/workflows/ci.yml`** — coverage gate threshold raised from `65%` to `70%`. Headroom: after v0.95.0 (`strtype` 13.7% → 60.1%) and v0.96.0 (eventtype/httpmethodtype/scripttype uplifts), `total` sits comfortably above 73%. Step name (`Coverage gate (70%)`), `awk` threshold, and error message updated in lock-step.

### Notes
- Next planned bump (→ 75%) is gated on AL2-08 (`osdetect` Linux/Windows guarded branches), which is the largest single-package drag still under 60%.
...
### Added
- `spec/05-failing-tests/README.md` — directory-level provenance disclaimer (25 upstream `core-v9` post-mortems; `tests/integratedtests/` paths describe upstream layout, NOT `enum-v10`).
...
### Changed
- `spec/07-code-vs-spec-audits/03-cycle2-error-system.md`: promoted all 7 ❓ rows against upstream `core-v9 v1.5.8` `errcore/`. Verifiable score 11 → **18/18 (100%)**, match rate 10/18 = 55.6%. No new D-CVS findings; spec is accurate, just under-exercised by `enum-v10`.
...
- `Test_AllEnums_NumericRange` line 86 failure is the known PI-006/RCA-Pattern-3 upstream `BasicString` spread defect (no enum-v10 regression; covered by existing skip-list).
...
## [v0.39.0] — 2026-05-07 — Cycle 67 — Bulk rename enum-v10 → enum-v10

### Changed
- Repository renamed from `enum-v10` to `enum-v10` per upstream rename. Bulk substitution applied across `go.mod`, Go source imports, all `spec/`, `.lovable/` memory files, workflows, and `cross-repo/core-v9/README.md` (one-time exception). `.release/`, `.git/`, and the `cross-repo/core-v9/` directory name preserved.
...
- **Cycle 65 (2026-05-06) — Repository rename `enum-v10` → `enum-v10`.** User renamed the GitHub repo a second time (after the v4 → v5 rename earlier the same day). Bulk `sed` rewrite applied across the entire main tree (~120 files): `go.mod` module path → `github.com/alimtvnetwork/enum-v10`, every Go source import (`cmd/main/main.go`, `inttype`, `linuxvendortype`, `osarchs`, `osdetect`, `promptclitype`, `quotes`, `strtype`, all `tests/creationtests/*`), every spec doc under `spec/01-app/`, `spec/02-app-issues/`, `spec/03-powershell-test-run/`, `spec/04-tooling/`, `spec/06-testing-guidelines/`, `spec/07-code-vs-spec-audits/`, `spec/00-llm-integration-guide.md`, `spec/CHANGELOG.md`, all `.lovable/` memory/plan/suggestions/pending-issues/cicd-issues files, root `CHANGELOG.md`/`CONTRIBUTING.md`, `.github/workflows/release.yml`, `.golangci.yml`, `run.sh`, `scripts/spec-api-check.psm1`, `tests/scripts/Test-ResolveTestSuiteRoot.ps1`. **Excluded:** `.release/` (frozen per Core memory), `.git/`, `node_modules/`, and `cross-repo/core-v9/` directory **name** (kept — it intentionally mirrors the older core-v8 upstream). Inside `cross-repo/core-v9/README.md` the enum-vN reference WAS bumped to enum-v10 per the established one-time exception (Core memory). Verification: `grep -rl 'enum-v10'` returns zero hits outside `.release/` and `.git/`; `core-v8` directory name still present (16 occurrences in its README, all referring to the upstream repo it mirrors). `package.json` 0.36.0 → 0.37.0. **No code logic changed**; only string substitution. Local `./run.ps1 -tc` should remain green provided the user's local clone is also renamed.
...
- **Cycle 19 spec audit (Task AB pass 1)** — upstream `core-v9 v1.5.8`
  ...
  (M-CVS-01: `enum-v10`→`enum-v10` module name; M-CVS-02: upstream `go.mod`
...
- **Cycle 18 spec audit (Task AA + closes Task AH)** — closed
  ...
  upstream-vs-`enum-v10` scope footnotes on the historical resolution files
...
- **Cycle 17 spec audit (Task AA + AH partial)** — closed `spec/04-tooling/`
  ...
  occurrence), and 4 stale `enum-v10`/`cross-repo/core-v9` tokens in
  `06-cross-repo-sync.md` (line 80 template comment carried both stale tokens).
  Each fix includes a Core-memory clarification that `cross-repo/core-v9/`
...
- **Cycle 15 spec audit (Task AA)** — closed `spec/06-testing-guidelines/`
  ...
  adding an `enum-v10` consumer-coverage callout to `spec/06-testing-guidelines/README.md`
...
- **core-v9 API migration (Task AM)** — Applied all confirmed `core-v9 v1.5.8`
  namespace rewrites across `enum-v10` Go source: `coredynamic.TypeName(...)` →
...
- **`scripts/CoverageRunner.psm1`** — pre-coverage compile check no longer
  ...
  2. The in-package-test scan hard-coded the `core-v9` module prefix when
     ...
     `module` line from `go.mod` so the same script works in `enum-v10`,
...
- Module path migrated from `gitlab.com/auk-go/enum` to
  `github.com/alimtvnetwork/enum-v10`.
