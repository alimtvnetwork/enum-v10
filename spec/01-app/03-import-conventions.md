...
For the full inventory of public packages, see [`01-package-map.md`](./01-package-map.md). Not every consumer uses every package — `enum-v10`, for example, currently uses 8 of the 11 listed canonical imports.
...
> **Note (consumer repos like `enum-v10`):** This `internal/` access does **not** cross modules. A consumer (e.g. `enum-v10`) cannot import `core-v9/internal/...` because Go's `internal/` rule is enforced at module boundaries, not just package boundaries. Consumer-side tests should depend only on `core-v9`'s public API. See the upstream `core-v9` repo for examples of in-module `internal/` test imports.
...
Test packages live under the repo's `tests/` directory in a sub-folder named after the test suite. In **`enum-v10`**, the layout is:
