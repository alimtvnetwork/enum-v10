...
> **Consumer-coverage note (`enum-v10`)**: every layout, wrapper, and helper described on this page (`tests/integratedtests/`, `tests/testwrappers/`, `coretests.GetAssert`, `coretestcases.CaseV1`, `StringsTestWrapper`, etc.) refers to **upstream `core-v9`**. `enum-v10` does not consume any of them — `rg tests/testwrappers` and `rg coretests.GetAssert` over `enum-v10` source both return zero hits. This module's tests live at `tests/creationtests/` (one shared package, Goconvey-based registry over `EnumTestWrapper`); see [`13-testing-patterns.md` §6.1](./13-testing-patterns.md#61-enum-v10-specific-layout) for that file-by-file layout. Treat §§1–5 below as the authoritative reference for upstream `core-v9`.
...
> ⚠️ **Scope:** the layout below applies to **upstream `core-v9`**. `enum-v10` uses a single shared `tests/creationtests/` package — see [`13-testing-patterns.md` §6.1](./13-testing-patterns.md#61-enum-v10-specific-layout). Prior fixes that removed stale `tests/integratedtests/` references from the spec corpus: C-CVS-01 / D-CVS-17 / D-CVS-26 / D-CVS-27 / D-CVS-32 / D-CVS-36 — this section is the 7th occurrence (D-CVS-39).
...
```
tests/creationtests/widgettests/    # ← upstream core-v9 layout; in enum-v10 register the enum in tests/creationtests/allBasicEnumsCollection.go instead
