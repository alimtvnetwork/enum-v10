...
Created `tests/contracttests/converters_test.go` (package `contracttests`) — a new test directory dedicated to behavioural contract claims that span enum-v10 → upstream `core-v9` boundaries. Five tests, each pinning one spec claim:
