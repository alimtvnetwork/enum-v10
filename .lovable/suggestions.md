...
- **Priority:** Low
- **Description:** Cycle 13 introduced spec-internal-consistency as an explicit verifiability dimension. `errcore.VarTwoNoType` was scored ❓ in Cycle 6 row 16 because no `enum-v10` consumer imports it, but it IS cross-referenced from `04-error-system.md:131`, `08-validators.md:240,307,329`, and `15-observability.md`. Under the new dimension it would promote to ✅. Backport mechanically when Task AC runs.
...
- **Notes:** Spec §12 used to assert "no `cmd/` directory anywhere"; reality: `enum-v10/cmd/main/main.go` is a single permitted smoke-test harness. §12 §1 rewritten as a "library-first, smoke-test allowed" policy. Cross-link to `cmd/README.md` added.
...
- **Notes:** Original §06 §3 / §5 referenced symbols that don't exist. Rewrote around the actual top-level constructors used by `enum-v10`.
...
- **Notes:** Three sections now carry explicit "this surface has no `enum-v10` consumer; verify via Task AB" callouts so future readers don't assume verified ✅ status incorrectly.
