# 02 — Go's `internal/` Rule Rejects `enum-v10` Consumers
...
…even though `enum-v10` source imports the package as `github.com/alimtvnetwork/core-v9/.../internal/X`.

## Root Cause

Go's `internal/` rule is enforced against the **cached module's declared path** (`core-v8`), NOT the import path the consumer used (`core-v9`). To the toolchain, `enum-v10` looks like a foreign module reaching into a different module's `internal/`.
