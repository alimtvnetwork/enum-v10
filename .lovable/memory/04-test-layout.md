...
Tests live under `tests/creationtests/` in `enum-v9`. The structure is a flat Goconvey + `EnumTestWrapper` registry, NOT a per-package layout.

## Stale spec pattern

The string `tests/integratedtests/` appears in spec docs as an upstream-only convention from `core-v9`. It is **not** the layout used by `enum-v9`. Audit findings:
...
Test-discovery tooling MUST accept either folder name (or read from disk). Hard-coding one name will silently break either upstream or `enum-v9`.
