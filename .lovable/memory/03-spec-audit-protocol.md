...
Drive `spec/` to factual truth by auditing each section against `enum-v10`'s actual code, then closing drift via spec edits (preferred) or escalating to the user.
...
2. **Probe code** — run `rg` against `enum-v10` Go source (excluding `cross-repo/`, `tests/`, `cmd/`) for each named symbol or pattern.
...
- **`tests/integratedtests/`** appeared 9 times across `01-app/` (cycles 1, 3, 6, 8, 9, 10, 11, 12). Always corrected to `tests/creationtests/` (the real `enum-v10` layout) with cross-link to predecessors.
