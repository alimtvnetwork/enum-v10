...
## Core
This is a Go enum library (**enum-v9**, module path `github.com/alimtvnetwork/enum-v9` — renamed from `enum-v6`, previously `enum-v5`, `enum-v4`, `enum-v3`, `enum-v2`, originally `enum-v1`). The frontend React/Vite shell is incidental — work happens in Go packages and PowerShell/Python tooling.
...
The `cross-repo/core-v9/` directory mirrors CI surface to a separate upstream repo and intentionally keeps its `core-v8` name. Do not rename it. As of 2026-05-06, `enum-v3` → `enum-v9` rename WAS applied inside `cross-repo/` per user direction (one-time exception to the usual leave-cross-repo-alone rule).
