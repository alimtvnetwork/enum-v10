...
- **Status:** in-progress (3/7 done as of v1.36.0).
- **Created:** 2026-05-10 (post-remix CI failure on per-package coverage gate).
- **Source:** Lovable / CI failure.
- **Affected (still ignored):** `dbdrivertype` 58.8%, `instructiontype` 68.4%, `inttype` 66.4%, `osarchs` 72.5%.
- **Done:** ✅ `httpstatusfamily` (v1.34.0). ✅ `compressformats` (v1.35.0). ✅ `compresslevels` 71.2% → ≥95% (v1.36.0, new `CompressLevels_DirectMethods_test.go`).
