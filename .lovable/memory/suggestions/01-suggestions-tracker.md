...
- **Status:** in-progress (4/7 done as of v1.37.0).
- **Created:** 2026-05-10 (post-remix CI failure on per-package coverage gate).
- **Source:** Lovable / CI failure.
- **Affected (still ignored):** `dbdrivertype` 58.8%, `instructiontype` 68.4%, `inttype` 66.4%.
- **Done:** ✅ `httpstatusfamily` (v1.34.0). ✅ `compressformats` (v1.35.0). ✅ `compresslevels` (v1.36.0). ✅ `osarchs` 72.5% → ≥95% (v1.37.0, new `OsArchs_DirectMethods_test.go`).
