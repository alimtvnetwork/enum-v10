...
- **affectedProject:** enum-v10 tooling
...
- **source:** Lovable (reliability report, 2026-05-05)
- **resolution:** Verified obsolete. `spec/06-testing-guidelines/01-folder-structure.md` line 3 already carries the upstream-scope disclaimer added in earlier audit cycles: *"⚠️ Scope: the layout below describes upstream `core-v9`. enum-v10 uses a single shared `tests/creationtests/` package — see [spec/01-app/13-testing-patterns.md §6.1]…"*. The `integratedtests` references in the file body intentionally document the upstream consumer layout, not enum-v10. Closing as **resolved-by-prior-cycle** with no edit needed.
...
- **resolution:** Stopgap no longer relevant. The underlying problem (Go 1.25 rejecting the dual-path `replace` bridge to upstream `core-v9`) was fixed at the source by Tasks W + AG (2026-05-05): upstream `core-v9` was renamed to `module github.com/alimtvnetwork/core-v9` and tagged `v1.5.8`; `enum-v10/go.mod` now pins `core-v9 v1.5.8` directly with no `replace` bridge (per Core memory). Pinning to Go 1.22 would now mask the working modern setup. **Do not** add `toolchain go1.22.0` to `go.mod`.
- **acceptance criteria:** ✅ `replace` bridge removed (Task AG). ✅ `enum-v10` builds against current toolchain without the stopgap. ✅ Risk noted in original suggestion (toolchain lock-in) avoided.
